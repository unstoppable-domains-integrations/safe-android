...
class TransactionPagingProvider(
    ...
    fun getTransactionsStream(safe: Solidity.Address, history: Boolean = true): Flow<PagingData<UnifiedEntry>> {
        return Pager(
            config = PagingConfig(pageSize = NETWORK_PAGE_SIZE, enablePlaceholders = false),
            pagingSourceFactory = { TransactionPagingSource(safe, transactionRepository, history) }
        ).flow
    }
